...
var TestStateMixin = Mixin.create({
  entered: 0,
  enter() {
    this.entered++;
  },
...
module('StateManager', function(hooks) {
  ...
  test("it exists", (assert) => {
    assert.ok(EmberObject.detect(StateManager), "StateManager is an Ember.Object");
  });
...
module("StateManager - Transitions on Complex State Managers", function() {
  /**
              SM
            /    \
       Login      Redeem
...
module("StateManager - Event Dispatching", function(hooks) {
...
module("Statemanager - Pivot states", function(hooks) {
  hooks.beforeEach(function() {
...
module("Transition event", function() {
